    ...
    <div className="min-h-screen flex flex-col lg:flex-row bg-gradient-to-br from-blue-50 via-white to-emerald-50">
      ...
      <main className="flex-1 p-4 lg:p-8 min-w-0">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-8">
          <StatCard icon={<Copy className="w-5 h-5" />} tint="bg-blue-100 text-blue-600" label="My Reports" value={reports.length} />
              ...
              {reports.slice(0, 5).map((r) => (
                <li key={r.id} className="bg-white rounded-xl p-4 shadow-sm flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3 min-w-0">
                    <div className="w-10 h-10 rounded-lg bg-blue-50 text-blue-500 flex items-center justify-center text-xs font-bold">{r.report_type === "high" ? "9-10" : "6-8"}</div>
                    <div className="min-w-0">
                      <div className="font-semibold text-sm truncate">{r.student_name || "Untitled"}</div>
