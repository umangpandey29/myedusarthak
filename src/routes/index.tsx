...
export const Route = createFileRoute("/")({
  ...
  head: () => ({
    meta: [
      { title: "MyEduSarthak — Create Report Card" },
      { name: "description", content: "Generate Hindi school marksheets with live preview and image export." },
    ],
  }),
});
...
function CreateReport() {
  ...
  const rows = useMemo(() => marks.map((m) => {
    ...
    const totalObtained = halfObtained + annObtained;
    const totalMax = n(m.hMax) + n(m.aMax);
  ...
  const reset = () => {
    ...
    setStudent({ name: "", father: "", mother: "", classSec: "", rollNo: "", dob: "",
      session: "2026-2027", janpadCode: "", schoolCode: "", srNo: "",
      schoolName: "जयप्रकाश नारायण सर्वोदय विद्यालय, तड़सड़ा (कठिराँव), जनपद-वाराणसी" });
  };
      ...
      <main className="flex-1 p-6">
        <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
          ...
          <div className="flex gap-2">
            ...
            <Button size="sm" onClick={download} className="bg-primary hover:bg-primary/90">
              <Download className="w-4 h-4 mr-1" />Save & Download
            </Button>
          </div>
        </div>
                  ...
                  <div key={i} className="border border-border rounded-lg p-3 bg-secondary/30">
                    ...
                    <div className="grid grid-cols-2 gap-1.5">
                      ...
                      <Input
                        ...
                        placeholder="Grade (A1, B2…) — not added to total"
                        value={marks[i].grade}
